...
# stripe listen --forward-to localhost:8000/pment/stripe/webhook/
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if payment.get('state') == 0:
                ...
                raise Exception(f'Error creating Helecet payment: status ={response.status_code}, response={response.text}')
        else:
...
@csrf_exempt
@require_POST
def helecet_webhook(request):
    if request.method != 'POST':
        return HttpResponseNotAllowed(['POST'])

    try:
        signature = request.headers.get('sigm')
